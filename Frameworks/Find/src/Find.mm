...
- (NSGridView*)gridView
{
	if(!_gridView)
	{
		NSTextField* findLabel              = OakCreateLabel(@"查找:");
		NSButton* findHistoryButton         = OakCreateHistoryButton(@"显示查找记录");

		NSButton* countButton               = OakCreateButton(@"Σ", NSBezelStyleSmallSquare);
		countButton.toolTip                 = @"显示结果统计";
		countButton.accessibilityLabel      = countButton.toolTip;

		NSTextField* replaceLabel           = OakCreateLabel(@"替换:");
		NSButton* replaceHistoryButton      = OakCreateHistoryButton(@"Show Replace History");

		NSTextField* optionsLabel           = OakCreateLabel(@"选项:");

		NSButton* ignoreCaseCheckBox        = OakCreateCheckBox(@"忽略大小写");
		NSButton* ignoreWhitespaceCheckBox  = OakCreateCheckBox(@"忽略空格");
		NSButton* regularExpressionCheckBox = OakCreateCheckBox(@"正则表达式");
		NSButton* wrapAroundCheckBox        = OakCreateCheckBox(@"循环");

		NSTextField* whereLabel             = OakCreateLabel(@"位置:");
		_wherePopUpButton                   = OakCreatePopUpButton(NO, nil, whereLabel);
		NSTextField* matchingLabel          = OakCreateLabel(@"匹配");
		...
		MBMenu const items = {
			{ /* Placeholder */ },
			{ @"搜索",                               @selector(nop:)                                    },
			{ @"二进制文件",                         @selector(toggleSearchBinaryFiles:),   .indent = 1 },
			{ @"隐藏文件夹",                       @selector(toggleSearchHiddenFolders:), .indent = 1 },
			{ @"符号链接的文件夹",            @selector(toggleSearchFolderLinks:),   .indent = 1 },
			{ @"符号链接的文件",              @selector(toggleSearchFileLinks:),     .indent = 1 },
			{ /* -------- */ },
			{ @"折叠结果",                     @selector(toggleCollapsedState:),      @"1", .modifierFlags = NSEventModifierFlagCommand|NSEventModifierFlagOption, .target = _resultsViewController },
			{ @"选择结果",                        .delegate = self                                   },
			{ /* -------- */ },
			{ @"复制匹配词组",                  @selector(copyMatchingParts:)                      },
			{ @"复制匹配词组和文件名",   @selector(copyMatchingPartsWithFilename:)          },
			{ @"复制匹配行",                    @selector(copyEntireLines:)                        },
			{ @"复制匹配行和文件名",     @selector(copyEntireLinesWithFilename:)            },
			{ @"复制替换内容",                    @selector(copyReplacements:)                       },
			{ /* -------- */ },
			{ @"选择所有",                            @selector(checkAll:)                               },
			{ @"反选所有",                          @selector(uncheckAll:)                             },
		};
...
- (NSStackView*)actionButtonsStackView
{
	...
	{
		_findAllButton                 = OakCreateButton(@"查找所有");
		NSButton* replaceAllButton     = OakCreateButton(@"替换所有");
		NSButton* replaceButton        = OakCreateButton(@"替换");
		NSButton* replaceAndFindButton = OakCreateButton(@"替换/查找");
		NSButton* findPreviousButton   = OakCreateButton(@"上一个");
		_findNextButton                = OakCreateButton(@"下一个");
...
- (void)updateWindowTitle
{
	if(NSString* folder = self.searchFolder)
		self.window.title = [NSString localizedStringWithFormat:@"查找%@", [folder stringByAbbreviatingWithTildeInPath]];
	else if(_searchTarget == FFSearchTargetOpenFiles)
		self.window.title = @"打开文件";
	else
		self.window.title = @"查找";
}
...
- (void)updateSearchInPopUpMenu
{
	...
	MBMenu const items = {
		{ @"文档",            @selector(orderFrontFindPanel:),  @"f", .tag = FFSearchTargetDocument          },
		{ @"选择的",           @selector(orderFrontFindPanel:),        .tag = FFSearchTargetSelection         },
		{ /* -------- */ },
		{ @"打开的文件",          @selector(orderFrontFindPanel:),        .tag = FFSearchTargetOpenFiles         },
		{ @"项目文件夹",      @selector(orderFrontFindPanel:),  @"F", .tag = FFSearchTargetProject           },
		{ @"文件浏览项目",  @selector(orderFrontFindPanel:),        .tag = FFSearchTargetFileBrowserItems  },
		{ @"其他文件夹…",       @selector(showFolderSelectionPanel:),   .tag = FFSearchTargetOther             },
		{ /* -------- */ },
		{ @"«上一个文件夹»",       @selector(orderFrontFindPanel:),        .ref = &folderItem                     },
		{ /* -------- */ },
		{ @"最近位置",       @selector(nop:),                                                               },
	};
...
- (NSString*)replaceAllButtonTitle   { return _showsResultsOutlineView && (_countOfExcludedMatches || _countOfReadOnlyMatches && _countOfReadOnlyMatches != _countOfMatches) ? @"替换选择的" : @"替换全部"; }
...
- (void)didFind:(NSUInteger)aNumber occurrencesOf:(NSString*)aFindString atPosition:(text::pos_t const&)aPosition wrapped:(BOOL)didWrap
{
	static std::string const formatStrings[4][3] = {
		{ "“${found}”没有更多匹配项", "发现“${found}”${line:+ 在第${line}行, 第${column}列}.", "“${found}”有${count}个匹配项" },
		{ "“${found}”没有更多匹配项", "发现“${found}”${line:+ 在第${line}行, 第${column}列}.", "“${found}”有${count}个匹配项" },
	};
...
- (void)didReplace:(NSUInteger)aNumber occurrencesOf:(NSString*)aFindString with:(NSString*)aReplacementString
{
	static NSString* const formatStrings[2][3] = {
		{ @"没有替换(“%@”没有匹配项).", @"“%@”的1个匹配项已被替换", @"“%@”的%2$@个匹配项已被替换" },
		{ @"没有替换(“%@”没有匹配项).", @"“%@”的1个匹配项已被替换", @"“%@”的%2$@个匹配项已被替换" }
	};
...
- (void)folderSearchDidFinish:(NSNotification*)aNotification
{
	...
	{
		case 0:  fmt = @"“%@”无匹配项.";     break;
		case 1:  fmt = @"“%@”发现一个匹配项.";     break;
		default: fmt = @"“%1$@”发现%2$@个匹配项."; break;
	}
	...
	if(_documentSearch)
	{
		...
		self.statusString          = [msg stringByAppendingFormat:([_documentSearch scannedFileCount] == 1 ? @" (搜索一个文件用了%@秒)" : @" (搜索%2$@个文件用了%1$@秒)"), seconds, [NSNumberFormatter localizedStringFromNumber:@([_documentSearch scannedFileCount]) numberStyle:NSNumberFormatterDecimalStyle]];
		self.alternateStatusString = [msg stringByAppendingFormat:@" (搜索%2$@个文件用了%1$@秒)", seconds, [NSByteCountFormatter stringFromByteCount:_documentSearch.scannedByteCount countStyle:NSByteCountFormatterCountStyleFile]];
	}
	else
	{
...
- (void)observeValueForKeyPath:(NSString*)keyPath ofObject:(id)object change:(NSDictionary*)change context:(void*)context
{
	if([keyPath isEqualToString:@"currentPath"])
	{
		...
		self.statusString = [NSString localizedStringWithFormat:@"搜索中 “%@”…", [NSString stringWithCxxString:relative]];
	}
}
...
- (void)didRemoveResult:(FFResultNode*)item
{
	...
	{
		case 0:  fmt = @"“%@”无匹配项.";             break;
		case 1:  fmt = @"“%@”显示一个结果.";     break;
		default: fmt = @"“%1$@”显示%2$@个结果."; break;
	}
...
- (void)updateShowTabMenu:(NSMenu*)aMenu
{
	...
	{
		[[aMenu addItemWithTitle:@"无结果" action:@selector(nop:) keyEquivalent:@""] setEnabled:NO];
	}
	else
	{
		char key = 0;
