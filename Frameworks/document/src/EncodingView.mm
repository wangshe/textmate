...
- (instancetype)initWithData:(NSData*)data
{
	if(self = [super initWithWindow:[[NSWindow alloc] initWithContentRect:NSZeroRect styleMask:(NSWindowStyleMaskTitled|NSWindowStyleMaskClosable|NSWindowStyleMaskResizable|NSWindowStyleMaskMiniaturizable) backing:NSBackingStoreBuffered defer:NO]])
	{
		...
		self.title         = OakCreateLabel(@"未知编码", [NSFont boldSystemFontOfSize:0]);
		self.explanation   = OakCreateLabel();
		self.label         = OakCreateLabel(@"编码:");
		...
		self.learnCheckBox = OakCreateCheckBox(@"使用文档来训练编码分类器");
		self.openButton    = OakCreateButton(@"打开");
...
- (void)setDisplayName:(NSString*)aString
{
	...
	self.explanation.stringValue = [NSString stringWithFormat:@"文件“%@”中包含未知编码的字符，\n请选择用于打开该文件的正确编码。\n文件内容如下所示，并高亮显示了相关行。\n在继续之前，请检查所选择的编码预览看起来正常。", _displayName];
}
