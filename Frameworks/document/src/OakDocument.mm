...
- (void)saveModalForWindow:(NSWindow*)aWindow completionHandler:(void(^)(OakDocumentIOResult result, NSString* errorMessage, oak::uuid_t const& filterUUID))block
{
	...
	if(_buffer)
	{
		...
		struct callback_t : file::save_callback_t
		{
			...
			void select_make_writable (std::string const& path, io::bytes_ptr content, file::save_context_ptr context)
			{
				...
				NSAlert* alert = [NSAlert tmAlertWithMessageText:[NSString stringWithFormat:@"文件“%@”被锁定", _document.displayName] informativeText:@"Do you want to overwrite it anyway?" buttons:@"Overwrite", @"Cancel", nil];
			...
			void select_create_parent (std::string const& path, io::bytes_ptr content, file::save_context_ptr context)
			{
				...
				[NSNotificationCenter.defaultCenter postNotificationName:OakDocumentWillShowAlertNotification object:_document];
				NSAlert* alert = [NSAlert tmAlertWithMessageText:[NSString stringWithFormat:@"“%@”没有上层文件夹.", _document.displayName] informativeText:[NSString stringWithFormat:@"您要在“%@”创建一个文件夹吗?", [NSString stringWithCxxString:path::with_tilde(path::parent(path))]] buttons:@"Create Folder", @"Cancel", nil];
				[alert beginSheetModalForWindow:_window completionHandler:^(NSModalResponse returnCode){
					if(returnCode == NSAlertFirstButtonReturn)
			...
			void select_charset (std::string const& path, io::bytes_ptr content, std::string const& charset, file::save_context_ptr context)
			{
				if(charset != kCharsetNoEncoding)
				{
					...
					[NSNotificationCenter.defaultCenter postNotificationName:OakDocumentWillShowAlertNotification object:_document];
					NSAlert* alert = [NSAlert tmAlertWithMessageText:[NSString stringWithFormat:@"无法保存“%@”使用“%@”作为编码", _document.displayName, to_ns(charset)] informativeText:@"您要选择另一个编码吗:" buttons:@"保存", @"取消", nil];
	...
	{
		NSString* errorMessage = [NSString stringWithFormat:@"无法保存‘%@’: 没有内容，已经备份%s,已编辑%s.", self.displayName, _backupPath ? "YES" : "NO", self.isDocumentEdited ? "YES" : "NO"];
