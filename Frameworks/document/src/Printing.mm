...
- (id)init
{
	if((self = [super init]))
	{
		...
		NSTextField* themesLabel    = OakCreateLabel(@"主题:");
		NSPopUpButton* themes       = OakCreatePopUpButton();
		NSTextField* fontSizesLabel = OakCreateLabel(@"文字大小:");
		NSPopUpButton* fontSizes    = OakCreatePopUpButton();
		NSButton* printHeaders      = OakCreateCheckBox(@"打印页眉和页脚");
		...
		if(ordered.empty())
			[themesMenu addItemWithTitle:@"没有主题载入" action:@selector(nop:) keyEquivalent:@""];
