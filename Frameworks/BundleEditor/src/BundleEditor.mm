...
namespace
{
	...
	{
		{ bundles::kItemTypeBundle,       "description", "text.html.basic",                "tmBundle",       "bundle",         @"attr.bundle-editor.bundle",         @"BundleProperties",     @"插件"       },
		{ bundles::kItemTypeCommand,      "command",     NULL_STR,                         "tmCommand",      "command",        @"attr.bundle-editor.command",        @"CommandProperties",    @"命令"      },
		{ bundles::kItemTypeDragCommand,  "command",     NULL_STR,                         "tmDragCommand",  "dragCommand",    @"attr.bundle-editor.command.drop",   @"FileDropProperties",   @"拖动命令" },
		{ bundles::kItemTypeSnippet,      "content",     "text.tm-snippet",                "tmSnippet",      "snippet",        @"attr.bundle-editor.snippet",        @"SnippetProperties",    @"片段"      },
		{ bundles::kItemTypeSettings,     "settings",    "source.plist.textmate.settings", "tmPreferences",  "settings",       @"attr.bundle-editor.settings",       nil,                     @"设置"     },
		{ bundles::kItemTypeGrammar,      NULL_STR,      "source.plist.textmate.grammar",  "tmLanguage",     "grammar",        @"attr.bundle-editor.grammar",        @"GrammarProperties",    @"语法"      },
		{ bundles::kItemTypeProxy,        "content",     "text.plain",                     "tmProxy",        "proxy",          @"attr.bundle-editor.proxy",          nil,                     @"代理"        },
		{ bundles::kItemTypeTheme,        NULL_STR,      "source.plist",                   "tmTheme",        "theme",          @"attr.bundle-editor.theme",          @"ThemeProperties",      @"主题"        },
		{ bundles::kItemTypeMacro,        "commands",    "source.plist",                   "tmMacro",        "macro",          @"attr.bundle-editor.macro",          @"MacroProperties",      @"宏指令"        },
	};
...
- (id)init
{
	if(self = [super initWithWindow:nil])
	{
		...
		self.windowFrameAutosaveName = @"插件编辑器";
		...
		self.splitViewController.splitView.autosaveName = @"插件编辑器";
		...
		self.windowSplitViewController.splitView.autosaveName = @"插件编辑器属性";
...
- (void)newDocument:(id)sender
{
	...
	NSMenu* menu = [[NSMenu alloc] initWithTitle:@"项目类型"];
	for(auto const& it : item_infos)
	{
	...
	NSAlert* alert = [NSAlert tmAlertWithMessageText:@"创建新项目" informativeText:@"请选择您要创建的内容:" buttons:@"创建", @"取消", nil];
...
- (BOOL)commitEditing
{
	...
	if(info.plist_key == NULL_STR || oak::contains(std::begin(PlistItemKinds), std::end(PlistItemKinds), info.kind))
	{
		...
		if(!success)
		{
			NSAlert* alert = [NSAlert tmAlertWithMessageText:@"解析属性列表时出错" informativeText:@"属性列表无效.\n\n不幸的是，目前无法指出解析器失败的地方" buttons:@"好", nil];
			[alert beginSheetModalForWindow:self.window completionHandler:^(NSModalResponse returnCode){ }];
			return NO;
		}
	}
...
- (void)saveDocument:(id)sender
{
	...
	if(!changes.empty())
	{
		NSAlert* alert = [NSAlert tmAlertWithMessageText:@"存储插件项目时出错" informativeText:@"抱歉，尝试存储您的更改时出了点问题。可以通过控制台获得更多信息。" buttons:@"好", nil];
		[alert beginSheetModalForWindow:self.window completionHandler:^(NSModalResponse returnCode){
...
- (void)browser:(NSBrowser*)aBrowser willDisplayCell:(id)aCell atRow:(NSInteger)aRow column:(NSInteger)aColumn
{
	if(NSBrowserCell* cell = [aCell isKindOfClass:[NSBrowserCell class]] ? aCell : nil)
	{
		...
		if(bundles::item_ptr item = entry->represented_item())
		{
			...
			{
				NSMenuItem* menuItem = [menu addItemWithTitle:@"导出插件…" action:@selector(exportBundle:) keyEquivalent:@""];
			...
			else if(paths.size() > 1)
			{
				...
				NSMenuItem* submenuItem = [menu addItemWithTitle:@"在访达中显示" action:nil keyEquivalent:@""];
				submenuItem.submenu = submenu;
			}

			NSMenuItem* menuItem = [menu addItemWithTitle:@"复制UUID" action:@selector(copyUUID:) keyEquivalent:@""];
			menuItem.target = self;
			menuItem.representedObject = [NSString stringWithCxxString:item->uuid()];
		}
		else
...
{
	NSMenuItem* item = [[NSMenuItem alloc] initWithTitle:[NSString stringWithFormat:@"在访达中显示“%@”", [NSString stringWithCxxString:path::display_name(path)]] action:@selector(showInFinder:) keyEquivalent:@""];
	item.target = self;
		...
		[savePanel beginSheetModalForWindow:self.window completionHandler:^(NSModalResponse result) {
			if(result == NSModalResponseOK)
			{
				...
				if(!res)
				{
					...
					alert.messageText     = @"存储插件失败";
					alert.informativeText = [NSString stringWithFormat:@"存储插件“%@”时出现未知错误", [path stringByAbbreviatingWithTildeInPath]];
...
{
	NSString* res = [NSString stringWithCxxString:text::format("您要存储更改到%zu吗?", changes.size())];
	if(changes.size() == 1)
	{
...
- (BOOL)windowShouldClose:(id)sender
{
	...
	[alert setInformativeText:@"如果不存储的话您的更改将会丢失"];
	[alert addButtons:@"存储", @"取消", @"复原更改", nil];
	[alert beginSheetModalForWindow:self.window completionHandler:^(NSModalResponse returnCode){
