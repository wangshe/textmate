...
- (void)loadView
{
	self.ownerLabel       = OakCreateLabel(@"所有者:", nil, NSTextAlignmentRight);
	self.ownerTextField   = OakCreateTextField();
	self.licenseLabel     = OakCreateLabel(@"许可证:", nil, NSTextAlignmentRight);
	...
	self.buyButton        = OakCreateButton(@"在线商店");
	self.cancelButton     = OakCreateButton(@"取消");
	self.registerButton   = OakCreateButton(@"添加许可证");
...
- (BOOL)addLicense:(License*)info
{
	...
	if(license::add(to_s([info.owner stringByTrimmingCharactersInSet:NSCharacterSet.whitespaceAndNewlineCharacterSet]), to_s(info.licenseAsBase32), &error))
	{
		...
		alert.messageText     = @"许可证已添加到钥匙串";
		alert.informativeText = @"感谢您的支持";
