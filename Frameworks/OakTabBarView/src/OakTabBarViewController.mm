...
- (void)updateGoToMenu:(NSMenu*)aMenu
{
	...
	{
		[aMenu addItemWithTitle:@"没有打开的选项卡n" action:@selector(nop:) keyEquivalent:@""];
	}
	else
	{
		[aMenu addItem:[NSMenuItem separatorItem]];

		NSMenuItem* item = [aMenu addItemWithTitle:@"上次的选项卡" action:@selector(takeSelectedTabIndexFrom:) keyEquivalent:@"9"];
