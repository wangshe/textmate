...
- (id)init
{
	if(self = [super initWithNibName:nil label:@"文件" image:[NSImage imageNamed:NSImageNameMultipleDocuments]])
	{
...
- (void)loadView
{
	NSButton* restoreDocumentsCheckBox       = OakCreateCheckBox(@"打开上次会话的文档");
	NSButton* createAtStartupCheckBox        = OakCreateCheckBox(@"创建一个文档");
	NSButton* createOnActivationCheckBox     = OakCreateCheckBox(@"重新激活时创建一个文档");
	...
	NSGridView* gridView = [NSGridView gridViewWithViews:@[
		@[ OakCreateLabel(@"启动时:"),             restoreDocumentsCheckBox   ],
		@[ NSGridCell.emptyContentView,                OakCreateLabel(@"按住(⇧)时跳过", smallFont) ],
		@[ OakCreateLabel(@"没有打开的文档时:"), createAtStartupCheckBox    ],
		@[ NSGridCell.emptyContentView,                createOnActivationCheckBox ],

		@[ ],

		@[ OakCreateLabel(@"新文档类型:"),      newDocumentTypesPopUp     ],
		@[ OakCreateLabel(@"未知文档类型:"),  unknownDocumentTypesPopUp ],
		@[ OakCreateLabel(@"编码:"),               encodingPopUp             ],
		@[ OakCreateLabel(@"行尾:"),           lineEndingsPopUp          ],
	]];
	...
	NSMenuItem* item = [unknownDocumentTypesMenu addItemWithTitle:@"提示类型" action:@selector(selectUnknownFileType:) keyEquivalent:@""];
