...
#endif
		return [NSString stringWithFormat:@"插件索引上次更新: %@", dateString];
	}
...
- (void)installBundle:(Bundle*)bundle
{
	...
	self.bundleInstallActivityText = [NSString stringWithFormat:@"正在安装插件‘%@’…", bundle.name];

	[BundlesManager.sharedInstance installBundles:@[ bundle ] completionHandler:^(NSArray<Bundle*>* bundles){
		if(!bundle.installed)
			self.bundleInstallActivityText = [NSString stringWithFormat:@"安装插件‘%@’错误", bundle.name];
		else if(bundles.count == 1)
			self.bundleInstallActivityText = [NSString stringWithFormat:@"插件‘%@’已安装", bundle.name];
		else if(bundles.count == 2)
			self.bundleInstallActivityText = [NSString stringWithFormat:@"插件‘%@’和一个依赖已安装", bundle.name];
		else
			self.bundleInstallActivityText = [NSString stringWithFormat:@"已安装插件‘%@’和%ld个依赖", bundle.name, bundles.count-1];
...
- (void)uninstallBundle:(Bundle*)bundle
{
	...
	self.bundleInstallActivityText = [NSString stringWithFormat:@"已卸载插件‘%@’", bundle.name];
}
@end
...
- (void)loadView
{
	...
	NSTableColumn* bundleTableColumn      = [self columnWithIdentifier:kTableColumnIdentifierBundleName  title:@"插件"      editable:NO  width:140 resizingMask:NSTableColumnUserResizingMask];
	...
	NSTableColumn* updatedTableColumn     = [self columnWithIdentifier:kTableColumnIdentifierUpdated     title:@"已更新"     editable:NO  width:90  resizingMask:NSTableColumnNoResizing];
	NSTableColumn* descriptionTableColumn = [self columnWithIdentifier:kTableColumnIdentifierDescription title:@"描述" editable:NO  width:140 resizingMask:NSTableColumnAutoresizingMask];
	...
	NSButton* updateBundlesCheckbox = [NSButton checkboxWithTitle:@"自动检查并安装更新" target:nil action:nil];
