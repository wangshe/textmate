...
- (NSArray<NSURL*>*)performOperation:(FBOperation)op sourceURLs:(NSArray<NSURL*>*)srcURLs destinationURLs:(NSArray<NSURL*>*)destURLs unique:(BOOL)makeUnique select:(BOOL)selectDestinationURLs
{
	...
	for(NSUInteger i = 0; i < total; ++i)
	{
		...
		if(!res)
		{
			if((op & (FBOperationLink|FBOperationCopy|FBOperationMove)) && [error.domain isEqualToString:NSCocoaErrorDomain] && error.code == NSFileWriteFileExistsError)
			{
				...
				alert.messageText             = [NSString stringWithFormat:@"您要替换“%@”吗?", [NSFileManager.defaultManager displayNameAtPath:destURL.path]];
				alert.informativeText         = [NSString stringWithFormat:@"另一个“%@”名称的文件已经存在该位置", [NSFileManager.defaultManager displayNameAtPath:destURL.path]];
				alert.suppressionButton.title = @"全部替换";
				alert.showsSuppressionButton  = i+1 < total;
				[alert addButtons:@"替换", @"停止", @"跳过", nil];
			...
			else if(op == FBOperationTrash && [error.domain isEqualToString:NSCocoaErrorDomain] && error.code == NSFeatureUnsupportedError)
			{
				NSAlert* alert                = [[NSAlert alloc] init];
				alert.alertStyle              = NSAlertStyleCritical;
				alert.messageText             = [NSString stringWithFormat:@"您确定要删除“%@”吗?", [NSFileManager.defaultManager displayNameAtPath:srcURL.path]];
				alert.informativeText         = @"该项目会被立刻删除";
				alert.suppressionButton.title = @"删除全部";
				alert.showsSuppressionButton  = i+1 < total;
				[alert addButtons:@"删除", @"停止", @"跳过", nil];
	...
	switch(op)
	{
		case FBOperationLink:      self.undoManager.actionName = [NSString stringWithFormat:@"创建链接到%@", itemDescription];   break;
		case FBOperationCopy:      self.undoManager.actionName = [NSString stringWithFormat:@"拷贝%@", itemDescription];          break;
		case FBOperationDuplicate: self.undoManager.actionName = [NSString stringWithFormat:@"复制%@", itemDescription];        break;
		case FBOperationMove:      self.undoManager.actionName = [NSString stringWithFormat:@"移动%@", itemDescription];          break;
		case FBOperationRename:    self.undoManager.actionName = [NSString stringWithFormat:@"重命名%@", itemDescription];           break;
		case FBOperationTrash:     self.undoManager.actionName = [NSString stringWithFormat:@"移动%@到废纸篓", itemDescription]; break;
		case FBOperationNewFile:   self.undoManager.actionName = @"新建文件";   break;
		case FBOperationNewFolder: self.undoManager.actionName = @"新建文件夹"; break;
	}
