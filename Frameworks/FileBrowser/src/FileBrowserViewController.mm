...
- (void)goToSCMDataSource:(id)sender
{
	...
	if([url.scheme isEqualToString:@"file"])
	{
		...
		else
		{
			...
			if(repository)
			{
				alert.messageText     = [NSString stringWithFormat:@"“%@”版本控制被禁用.", self.fileItem.localizedName];
				alert.informativeText = @"出于性能原因，文本编辑不会监控此文件夹的版本控制信息。";
			}
			else
			{
				alert.messageText     = [NSString stringWithFormat:@"“%@”版本控制不可用", self.fileItem.localizedName];
				alert.informativeText = @"在文本编辑可以向您显示状态之前，您需要使用您最喜欢的版本控制系统初始化该文件夹。";
			}
...
- (void)folderPopUpButtonWillPopUp:(NSNotification*)aNotification
{
	...
	[[menu addItemWithTitle:@"其他…" action:@selector(orderFrontGoToFolder:) keyEquivalent:@""] setTarget:self];

	FileItem* item = self.fileItem;
	if(NSURL* url = item.URL.filePathURL)
	{
		[menu addItem:[NSMenuItem separatorItem]];
		[[menu addItemWithTitle:[NSString stringWithFormat:@"使用“%@”作为项目文件夹", item.localizedName] action:@selector(takeProjectPathFrom:) keyEquivalent:@""] setRepresentedObject:url.path];
	}
}
...
- (void)updateMenu:(NSMenu*)menu
{
	...
	for(OakOpenWithApplicationInfo* app in _openWithMenuDelegate.applications)
	{
		if(app.isDefaultApplication)
		{
			...
			{
				openWithTitle  = [NSString stringWithFormat:@"以%@打开", app.name];
	...
	MBMenu const items = {
		{ @"打开",                    @selector(openSelectedItems:)           },
		{ openWithTitle,              @selector(openWithMenuAction:), .delegate = _openWithMenuDelegate, .representedObject = openWithAppURL },
		{ /* -------- */ },
		{ @"显示原版",           @selector(showOriginal:)                },
		{ @"显示封闭文件夹",   @selector(showEnclosingFolder:)         },
		{ @"显示包内容",   @selector(showPackageContents:)         },
		{ @"在访达中显示",          @selector(showSelectedEntriesInFinder:) },
		{ /* -------- */ },
		{ @"新建文件",                @selector(newDocumentInDirectory:), @"n", NSEventModifierFlagCommand|NSEventModifierFlagControl },
		{ @"新建文件夹",              @selector(newFolder:),              @"n", NSEventModifierFlagCommand|NSEventModifierFlagShift   },
		{ /* -------- */ },
		{ @"重命名",                  @selector(editSelectedEntries:)                },
		{ @"复制",               @selector(duplicateSelectedEntries:)           },
		{ @"快速查看",              @selector(toggleQuickLookPreview:)             },
		{ @"添加到收藏",        @selector(addSelectedEntriesToFavorites:)      },
		{ @"从收藏中移除",   @selector(removeSelectedEntriesFromFavorites:) },
		{ /* -------- */ },
		{ @"移动到废纸篓",           @selector(delete:)   },
		{ /* -------- */ .ref = &insertBundleItemsMenuItem },
		{ /* -------- */ },
		{ @"拷贝",                    @selector(copy:)                                                                            },
		{ @"拷贝路径名",        @selector(copyAsPathname:),      @"",  NSEventModifierFlagOption, .tag = kRequiresSelectionTag, .alternate = YES },
		{ @"粘贴",                   @selector(paste:),                      },
		{ @"移动项目到此处",         @selector(pasteNext:),                  },
		{ @"为项目创建链接",    @selector(createLinkToPasteboardItems:) },
		{ /* -------- */ },
		{ @"访达标签", .tag = kRequiresSelectionTag, .ref = &finderTagsMenuItem },
		{ /* -------- */ },
		{ @"撤销",                    @selector(undo:) },
		{ @"重做",                    @selector(redo:) },
		{ /* -------- */ },
...
- (BOOL)validateMenuItem:(NSMenuItem*)menuItem
{
	...
	else if(menuItem.action == @selector(toggleQuickLookPreview:))
	{
		if([QLPreviewPanel sharedPreviewPanelExists] && [QLPreviewPanel sharedPreviewPanel].isVisible)
			...
			menuItem.title = [NSString stringWithFormat:@"快速查看“%@”", self.previewableItems.firstObject.localizedName];
		else
			menuItem.title = [NSString stringWithFormat:@"快速查看%ld的项目", self.previewableItems.count];
	}
	else if(menuItem.action == @selector(toggleShowInvisibles:))
		menuItem.dynamicTitle = self.showExcludedItems ? @"隐藏不可见文件" : @"显示不可见文件";
	...
	{
		NSString* copyAsPathnameTitle = previewableItems.count > 1 ? @"拷贝%@为路径名" : @"拷贝%@为路径名";
		...
		struct { NSString* format; SEL action; } const menuTitles[] =
		{
			...
			{ @"在访达中显示%@",        @selector(showSelectedEntriesInFinder:)        },
			{ @"添加%@到收藏",      @selector(addSelectedEntriesToFavorites:)      },
			{ @"从收藏中移除%@", @selector(removeSelectedEntriesFromFavorites:) },
		};
		...
		for(auto const& info : menuTitles)
		{
			if(menuItem.target == self && menuItem.action == info.action)
			{
				...
				switch(previewableItems.count)
				{
					...
					default: items = [NSString stringWithFormat:@" %ld 个项目", previewableItems.count]; break;
				}
		...
		if([self.directoryURLForNewItems getResourceValue:&folderNameForNewItems forKey:NSURLLocalizedNameKey error:nil])
		{
			if(menuItem.action == @selector(newFolder:))
				menuItem.dynamicTitle = [NSString stringWithFormat:@"“%@”中新文件夹", folderNameForNewItems];
			...
			if(menuItem.action == @selector(paste:) && menuItem.target == self)
			{
				...
				if(count == 1)
						menuItem.dynamicTitle = [NSString stringWithFormat:@"在“%@”中粘贴项目", folderNameForNewItems];
				else	menuItem.dynamicTitle = [NSString stringWithFormat:@"粘贴%ld个项目到“%@”中", count, folderNameForNewItems];
			}
			else if(menuItem.action == @selector(pasteNext:) && menuItem.target == self)
			{
				NSInteger count = [self URLsFromPasteboard:NSPasteboard.generalPasteboard].count;
				if(count == 1)
						menuItem.dynamicTitle = [NSString stringWithFormat:@"移动项目到“%@”", folderNameForNewItems];
				else	menuItem.dynamicTitle = [NSString stringWithFormat:@"移动%ld个项目到“%@”", count, folderNameForNewItems];
			}
			else if(menuItem.action == @selector(createLinkToPasteboardItems:) && menuItem.target == self)
			{
				NSInteger count = [self URLsFromPasteboard:NSPasteboard.generalPasteboard].count;
				if(count == 1)
						menuItem.dynamicTitle = [NSString stringWithFormat:@"创建链接到“%@”", folderNameForNewItems];
				else	menuItem.dynamicTitle = [NSString stringWithFormat:@"创建%ld个链接到“%@”", count, folderNameForNewItems];
			}
		}
	}
