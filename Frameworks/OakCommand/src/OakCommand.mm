...
- (void)executeWithInput:(NSFileHandle*)inputFH outputHandler:(void(^)(std::string const& out, output::type placement, output_format::type format, output_caret::type outputCaret, std::map<std::string, std::string> const& environment))handler
{
	...
	_processIdentifier = run_command(_dispatchGroup, scriptPath, inputFH.fileDescriptor, _environment, directory, CFRunLoopGetCurrent(), hasHTMLOutput ? htmlOutHandler : stdoutHandler, stderrHandler, ^(int status) {
		...
		if(WIFSIGNALED(status))
			os_log_error(OS_LOG_DEFAULT, "接收%{public}s后终止进程", strsignal(WTERMSIG(status)));
		else if(!WIFEXITED(status))
			os_log_error(OS_LOG_DEFAULT, "进程异常终止%d", status);
		...
		{
			NSMutableDictionary* dict = [NSMutableDictionary dictionaryWithDictionary:@{
				NSLocalizedDescriptionKey: [NSString stringWithFormat:@"运行“%@”失败.", to_ns(_bundleCommand.name)],
			}];

			NSMutableArray* buttonLabels = [NSMutableArray arrayWithObject:@"好"];
			if(bundles::lookup(_bundleCommand.uuid))
				[buttonLabels addObject:@"编辑命令"];

			std::string output = text::trim(err + out);
			if(std::count(output.begin(), output.end(), '\n') > 7)
			{
				[buttonLabels addObject:@"显示完整输出"];
			...
			dict[NSLocalizedRecoverySuggestionErrorKey] = to_ns(output.empty() ? text::format("命令返回状态码%d.", rc) : output) ?: @"Command output is not UTF-8.";
	...
	switch(error.code)
	{
		...
		case OakCommandAbnormalTerminationError:
		{
			...
			else if(recoveryOptionIndex == 2)
			{
				...
				OakDocument* doc = [OakDocument documentWithString:commandOutput fileType:@"text.plain" customName:@"命令输出"];
...
- (void)startLoading
{
	...
	dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
		...
		@catch(NSException* e) {
			NSData* data = [[NSString stringWithFormat:@"<p>读取%@数据时产生异常</p>", e.reason] dataUsingEncoding:NSUTF8StringEncoding];
			dispatch_sync(dispatch_get_main_queue(), ^{
				if(!_stop)
					[self.client URLProtocol:self didLoadData:data];
			});
