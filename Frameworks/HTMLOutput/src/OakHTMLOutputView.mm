...
- (void)stopLoadingWithUserInteraction:(BOOL)askUserFlag completionHandler:(void(^)(BOOL didStop))handler
{
	...
	{
		NSAlert* alert = askUserFlag ? [NSAlert tmAlertWithMessageText:[NSString stringWithFormat:@"停止“%@”?", [NSURLProtocol propertyForKey:@"processName" inRequest:request]] informativeText:@"任务正在执行的作业将不会完成" buttons:@"停止", @"取消", nil] : nil;
