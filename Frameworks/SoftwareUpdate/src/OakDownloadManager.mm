...
- (instancetype)initWithURL:(NSURL*)url forReplacingURL:(NSURL*)localURL publicKeys:(NSDictionary<NSString*, NSString*>*)publicKeys completionHandler:(void(^)(NSURL* extractedArchiveURL, NSError* error))completionHandler
{
	if(self = [super init])
	{
		...
		_progress.localizedDescription = [NSString stringWithFormat:@"正在下载 %@…", url.lastPathComponent];
...
- (void)dealloc
{
	...
	if(_temporaryFileURL && ![NSFileManager.defaultManager removeItemAtURL:_temporaryFileURL error:&error])
		os_log_error(OS_LOG_DEFAULT, "无法移除%{public}@: %{public}@", _temporaryFileURL.path, error.localizedDescription);
}
...
- (NSFileHandle*)extractorFileHandle
{
	if(!_extractorFileHandle)
	{
		...
		if(@available(macos 10.13, *))
		{
			...
			{
				os_log_error(OS_LOG_DEFAULT, "无法启动tar: %{public}@", error.localizedDescription);
...
- (void)URLSession:(NSURLSession*)session task:(NSURLSessionTask*)dataTask didCompleteWithError:(NSError*)downloadError
{
	...
	{
		os_log_error(OS_LOG_DEFAULT, "下载失败%{public}@: %{public}@", dataTask.originalRequest.URL, error.localizedDescription);
	...
	{
		os_log_error(OS_LOG_DEFAULT, "验证签名失败");
		_completionHandler(nil, [NSError errorWithDomain:@"OakDownloadManager" code:0 userInfo:@{ NSLocalizedDescriptionKey: @"无法验证签名" }]);
	}
	else
	{
		dispatch_group_notify(_extractorDispatchGroup, dispatch_get_main_queue(), ^{
			...
			else
			{
				...
				os_log_error(OS_LOG_DEFAULT, "tar: %d异常退出", _extractorTask.terminationStatus);
				if(errorString)
					os_log_error(OS_LOG_DEFAULT, "%{public}@", errorString);
				if(outputString)
					os_log_error(OS_LOG_DEFAULT, "%{public}@", outputString);

				NSString* description = errorString ?: outputString ?: [NSString stringWithFormat:@"tar: %d异常退出", _extractorTask.terminationStatus];
...
- (BOOL)data:(NSData*)contentData hasValidBase64EncodedSignature:(NSString*)encodedSignature usingPublicKeyString:(NSString*)publicKeyString
{
	...
	{
		os_log_error(OS_LOG_DEFAULT, "无法解码签名: %{public}@", encodedSignature);
	}
	return res;
}
@end
