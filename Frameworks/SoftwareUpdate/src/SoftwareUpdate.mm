	...
	if(_automaticUpdateCheckEnabled = flag)
	{
		...
		[_updateCheckScheduler scheduleWithBlock:^(NSBackgroundActivityCompletionHandler completionHandler){
			if(NSDate* suspendUntil = [NSUserDefaults.standardUserDefaults objectForKey:kUserDefaultsSoftwareUpdateSuspendUntilKey])
			{
				...
				{
					os_log(OS_LOG_DEFAULT, "跳过版本检查: 暂停至%{public}@", suspendUntil);
			...
			[self checkForTestBuild:NO completionHandler:^(NSURL* remoteURL, NSString* remoteVersion, NSError* error){
				...
				if(error)
				{
					os_log(OS_LOG_DEFAULT, "坚持更新失败: %{public}@", error.localizedDescription);
					completionHandler(NSBackgroundActivityResultFinished);
				}
				else
				{
...
- (void)checkForTestBuild:(BOOL)testBuild completionHandler:(void(^)(NSURL* remoteURL, NSString* remoteVersion, NSError* error))completionHandler
{
	...
	if(!updateChannel)
		return completionHandler(nil, nil, [NSError errorWithDomain:@"SoftwareUpdate" code:0 userInfo:@{ NSLocalizedDescriptionKey: @"没有配置通道" }]);

	NSURL* url = _channels[updateChannel];
	if(!url)
		return completionHandler(nil, nil, [NSError errorWithDomain:@"SoftwareUpdate" code:0 userInfo:@{ NSLocalizedDescriptionKey: [NSString stringWithFormat:@"没有通道名称‘%@’.", updateChannel] }]);
...
- (BOOL)presentError:(NSError*)error
{
	...
	self.infoViewController.messageTextField.stringValue     = @"检查更新错误";
	self.infoViewController.informativeTextField.stringValue = error.localizedDescription;
	[self addButtonWithTitle:@"好"];
...
- (void)presentUIForBackgroundCheck:(BOOL)backgroundCheck remoteURL:(NSURL*)remoteURL remoteVersion:(NSString*)remoteVersion redownloadEnabled:(BOOL)allowRedownload
{
	...
	{
		self.infoViewController.messageTextField.stringValue     = @"新版本可用";
		self.infoViewController.informativeTextField.stringValue = [NSString stringWithFormat: @"%@ %@版本可以用，您正在使用%@版本，需要现在更新吗?", [NSBundle.mainBundle objectForInfoDictionaryKey:@"CFBundleName"], remoteVersion, localVersion];

		[self addButtonWithTitle:@"下载"];
		[self addButtonWithTitle:backgroundCheck ? @"稍后" : @"取消"];
		self.buttons.lastObject.keyEquivalent = @"\e";
	}
	else if(ordering == NSOrderedSame)
	{
		self.infoViewController.messageTextField.stringValue     = @"最新";
		self.infoViewController.informativeTextField.stringValue = [NSString stringWithFormat:@"您正在运行%@版本，是目前最新的版本。", remoteVersion];
	...
	{
		self.infoViewController.messageTextField.stringValue     = @"您正在使用测试版本";
		self.infoViewController.informativeTextField.stringValue = [NSString stringWithFormat:@"%@版本是最新的可用版本，您的版本是%@.", remoteVersion, localVersion];

		[self addButtonWithTitle:@"好"];
		[self addButtonWithTitle:[NSString stringWithFormat:@"降级到%@", remoteVersion]];
	}
	...
	[self runModalWithCompletionHandler:^BOOL(NSModalResponse response){
		if(response == (ordering == NSOrderedAscending ? NSAlertFirstButtonReturn : NSAlertSecondButtonReturn))
		{
			...
			{
				NSString* informativeText = [NSString stringWithFormat:@"%1$@运行在只读的文件系统中，无法被更新。\n\n如果您从网页下载%1$@然后在下载文件夹中安装可解决该问题。", [NSBundle.mainBundle objectForInfoDictionaryKey:@"CFBundleName"]];
				[self presentAlertWithMessage:@"只读文件系统" informativeText:informativeText buttonTitles:@[ @"OK" ] completionHandler:^BOOL(NSModalResponse returnCode){
...
- (void)downloadSoftwareUpdateAtURL:(NSURL*)downloadURL
{
	...
	id <NSProgressReporting> progressReporting = [OakDownloadManager.sharedInstance downloadArchiveAtURL:downloadURL forReplacingURL:NSBundle.mainBundle.bundleURL publicKeys:self.publicKeys completionHandler:^(NSURL* extractedArchiveURL, NSError* error){
		...
		if(extractedArchiveURL)
		{
			...
			self.progressViewController.messageTextField.stringValue = [NSString stringWithFormat:@"已下载%@", downloadURL.lastPathComponent];
	...
	self.buttons[0].title         = @"安装/重启";
	self.buttons[0].enabled       = NO;

	self.buttons[1].title         = @"取消";
...
- (void)takeURLToInstallFrom:(NSButton*)sender
{
	...
	{
		self.progressViewController.messageTextField.stringValue     = [NSString stringWithFormat:@"正在安装%@…", [NSBundle.mainBundle objectForInfoDictionaryKey:@"CFBundleName"]];
		...
		{
			self.progressViewController.messageTextField.stringValue = [NSString stringWithFormat:@"正在重启%@…", [NSBundle.mainBundle objectForInfoDictionaryKey:@"CFBundleName"]];
...
- (void)loadView
{
	_messageTextField     = [NSTextField labelWithString:@"新版本可用"];
...
- (void)loadView
{
	_messageTextField     = [NSTextField labelWithString:@"正在下载压缩包…"];
