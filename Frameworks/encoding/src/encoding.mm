...
namespace encoding
{
	...
	void classifier_t::load (std::string const& path)
	{
		try {
			...
			os_log_error(OS_LOG_DEFAULT, "加载‘%{public}s’时出现异常: %{public}s", path.c_str(), e.what());
		}
	}
	...
	void classifier_t::real_load (std::string const& path)
	{
		...
		if(fd != -1)
		{
			...
			{
				os_log_info(OS_LOG_DEFAULT, "跳过‘%{public}s’版本%u (expected %u)", path.c_str(), freq.getVersion(), kCapnpClassifierFormatVersion);
				return;
			}
