...
- (id)init
{
	if((self = [super initWithWindow:[[NSPanel alloc] initWithContentRect:NSZeroRect styleMask:(NSWindowStyleMaskTitled|NSWindowStyleMaskClosable|NSWindowStyleMaskResizable|NSWindowStyleMaskMiniaturizable) backing:NSBackingStoreBuffered defer:NO]]))
	{
		...
		self.commandLabel         = OakCreateLabel(@"命令:", nil, NSTextAlignmentRight);
		self.commandComboBox      = OakCreateComboBox();
		self.resultLabel          = OakCreateLabel(@"结果:", nil, NSTextAlignmentRight);
		self.resultPopUpButton    = OakCreatePopUpButton();
		self.executeButton        = OakCreateButton(@"执行");
		self.cancelButton         = OakCreateButton(@"取消");

		struct { NSString* title; NSString* keyEquivalent; NSInteger outputOption; } outputOptions[] =
		{
			{ @"替换输入",      @"1", output::replace_input },
			{ @"输入后插入", @"2", output::after_input   },
			{ @"新文档",       @"3", output::new_window    },
			{ @"工具提示",           @"4", output::tool_tip      },
		};
		...
		self.commandHistoryList   = [[OakHistoryList alloc] initWithName:@"通过历史命令过滤" stackSize:10 defaultItems:@"sort|uniq -c", @"seq 100", @"cat -n", nil];

		self.window.title         = @"通过命令过滤";
