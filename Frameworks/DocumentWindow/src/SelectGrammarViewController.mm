...
- (NSString*)labelString
{
	if(_grammar && _documentDisplayName)
		return [NSString stringWithFormat:@"您要安装“%@”插件吗? 这将会改善文档支持例如“%@”.", _grammar.bundle.name, _documentDisplayName];
	else if(_grammar)
		return [NSString stringWithFormat:@"您要安装“%@”插件吗? 这将会改善对该文档的支持.", _grammar.bundle.name];
	else
		return @"您要安装队该文档的额外支持吗?";
}
...
- (void)loadView
{
	...
	self.neverButton       = OakSmallButton(@"从不", @selector(didClickButton:), self, SelectGrammarResponseNever);
	self.notNowButton      = OakSmallButton(@"现在不", @selector(didClickButton:), self, SelectGrammarResponseNotNow);
	self.installButton     = OakSmallButton(@"安装", @selector(didClickButton:), self, SelectGrammarResponseInstall);
