...
static void show_command_error (std::string const& message, oak::uuid_t const& uuid, NSWindow* window = nil, std::string commandName = NULL_STR)
{
	...
	[alert setInformativeText:[NSString stringWithCxxString:message] ?: @"没有输出"];
	[alert addButtonWithTitle:@"好"];
	if(bundleItem)
		[alert addButtonWithTitle:@"编辑命令"];
...
+ (NSAlert*)saveAlertForDocuments:(NSArray<OakDocument*>*)someDocuments
{
	...
	if(someDocuments.count == 1)
	{
		...
		[alert setMessageText:[NSString stringWithFormat:@"您要存储对文件“%@”所做的更改吗?", document.displayName]];
		[alert setInformativeText:@"如果不存储的话您所做的更改将会丢失"];
	...
	else
	{
		...
		[alert setMessageText:@"您要存储对这些文件所做的更改吗?"];
		[alert setInformativeText:body];
		[alert addButtons:@"全部存储", @"取消", @"复原更改", nil];
	}
	return alert;
}
		...
		[document saveModalForWindow:self.window completionHandler:^(OakDocumentIOResult result, NSString* errorMessage, oak::uuid_t const& filterUUID){
			...
			else
			{
				if(result == OakDocumentIOResultFailure)
				{
					...
					else	[[NSAlert tmAlertWithMessageText:[NSString stringWithFormat:@"文件“%@”无法被存储", document.displayName] informativeText:(errorMessage ?: @"请查看控制台输出以寻找原因") buttons:@"好", nil] beginSheetModalForWindow:self.window completionHandler:nil];
				}
...
- (NSMenu*)menuForTabBarView:(OakTabBarView*)aTabBarView
{
	...
	MBMenu const items = {
		{ @"新新标签页",                  @selector(takeNewTabIndexFrom:),    .representedObject = newTabAtTab   },
		{ @"移动标签页到新窗口",   @selector(takeTabsToTearOffFrom:),  .representedObject = total > 1 ? clickedTab : [NSIndexSet indexSet] },
		{ /* -------- */ },
		{ @"关闭标签页",                closeSingleTabSelector,                                                                           .representedObject = clickedTab    },
		{ @"关闭其他标签页",         @selector(takeTabsToCloseFrom:),                                                                  .representedObject = otherTabs     },
		{ @"关闭右边标签页",  @selector(takeTabsToCloseFrom:),                                                                  .representedObject = rightSideTabs },
		{ @"关闭左边标签页",   @selector(takeTabsToCloseFrom:),    .modifierFlags = NSEventModifierFlagOption, .alternate = YES, .representedObject = leftSideTabs  },
	};
...
- (void)updateShowTabMenu:(NSMenu*)aMenu
{
	...
	{
		[aMenu addItemWithTitle:@"没有标签页" action:@selector(nop:) keyEquivalent:@""];
		return;
	}
	...
	if(i == 0)
	{
		[aMenu addItemWithTitle:@"没有打开的标签页" action:@selector(nop:) keyEquivalent:@""];
	}
	else
	{
		[aMenu addItem:[NSMenuItem separatorItem]];

		NSMenuItem* item = [aMenu addItemWithTitle:@"上次的标签页" action:@selector(takeSelectedTabIndexFrom:) keyEquivalent:@"9"];
...
- (BOOL)validateMenuItem:(NSMenuItem*)menuItem
{
	...
	if([menuItem action] == @selector(toggleFileBrowser:))
		[menuItem setTitle:self.fileBrowserVisible ? @"隐藏文件浏览" : @"显示文件浏览"];
	else if([menuItem action] == @selector(toggleHTMLOutput:))
	{
		...
		[menuItem setTitle:isVisibleAndKey ? @"隐藏HTML输出" : @"显示HTML输出"];
	...
	else if([menuItem action] == @selector(newDocumentInDirectory:))
	{
		...
		[menuItem setDynamicTitle:active ? [NSString stringWithFormat:@"“%@”中新建文件", [NSFileManager.defaultManager displayNameAtPath:self.fileBrowser.directoryURLForNewItems.path]] : @"新建文件"];
	}
	...
	else if([menuItem action] == @selector(revealFileInProject:) || [menuItem action] == @selector(revealFileInProjectByExpandingAncestors:))
	{
		...
		[menuItem setDynamicTitle:active ? [NSString stringWithFormat:@"选择“%@”", self.selectedDocument.displayName] : @"选择文档"];
	}
		...
		[menuItem setTitle:self.window.firstResponder == self.textView ? @"移动焦点到文件浏览" : @"移动焦点到文档"];
...
- (NSTouchBarItem*)touchBar:(NSTouchBar*)touchBar makeItemForIdentifier:(NSTouchBarItemIdentifier)identifier
{
	...
	if([identifier isEqualToString:kTouchBarTabNavigationIdentifier])
	{
		...
		res.customizationLabel = @"后面/前面的标签页";
	}
