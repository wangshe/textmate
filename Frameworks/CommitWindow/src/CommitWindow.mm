...
- (id)initWithOptions:(NSDictionary*)someOptions
{
	if((self = [super init]))
	{
		...
		self.window.frameAutosaveName = @"提交窗口";
		...
		_cancelButton = OakCreateButton(@"取消");
...
- (void)beginSheetModalForWindow:(NSWindow*)aWindow completionHandler:(void(^)(NSModalResponse returnCode))aCompletionHandler
{
	...
	OakDocument* commitMessage = [OakDocument documentWithString:message fileType:to_ns(fileType) customName:@"提交消息"];
...
- (void)setupPreviousCommitMessagesMenu
{
	...
	[menu removeAllItems];
	[menu addItemWithTitle:@"以前提交的消息" action:NULL keyEquivalent:@""];
	...
	if(NSArray* commitMessages = [NSUserDefaults.standardUserDefaults stringArrayForKey:kOakCommitWindowCommitMessages])
	{
		...
		NSMenuItem* item = [menu addItemWithTitle:@"清除菜单" action:@selector(clearPreviousCommitMessages:) keyEquivalent:@""];
		[item setTarget:self];
	}
	else
	{
		...
		dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
			...
			if(!success)
			{
				dispatch_async(dispatch_get_main_queue(), ^{
					NSAlert* alert = [NSAlert tmAlertWithMessageText:@"运行diff命令失败" informativeText:[NSString stringWithCxxString:cmdString] buttons:@"好", nil];
...
- (void)performActionCommand:(id)sender
{
	...
	if(res != NULL_STR)
	{
		...
		{
			NSAlert* alert = [NSAlert tmAlertWithMessageText:@"无法理解命令的输出" informativeText:[NSString stringWithCxxString:cmdString] buttons:@"OK", nil];
			[alert beginSheetModalForWindow:self.window completionHandler:^(NSModalResponse returnCode){}];
		}
	...
	{
		NSAlert* alert = [NSAlert tmAlertWithMessageText:@"运行命令失败" informativeText:[NSString stringWithCxxString:cmdString] buttons:@"好", nil];
		[alert beginSheetModalForWindow:self.window completionHandler:^(NSModalResponse returnCode){}];
	}
}
...
- (void)menuNeedsUpdate:(NSMenu*)menu
{
	...
	if([_tableView clickedColumn] == -1)
		[menu addItemWithTitle:@"假的" action:NULL keyEquivalent:@""];

	NSInteger row = [_tableView clickedRow];
	if(row == -1 && [_tableView selectedRow] == -1)
	{
		[menu addItemWithTitle:@"查看全部" action:@selector(checkAll:) keyEquivalent:@""];
		[menu addItemWithTitle:@"不查看" action:@selector(uncheckAll:) keyEquivalent:@""];
	}
	else
	{
...
- (id)init
{
	if(self = [super init])
	{
		...
		if([_connection registerName:serviceName] == NO)
			os_log_error(OS_LOG_DEFAULT, "设置连接失败‘%@’", serviceName);
	}
	return self;
}
