...
_OutputIter copy_menu_items (NSMenu* menu, _OutputIter out, NSArray* parentNames = @[ ])
{
	for(NSMenuItem* item in [menu itemArray])
	{
		...
		if(NSMenu* submenu = [item submenu])
		{
			...
			{
				if([@[ @"拼写" ] containsObject:submenu.title])
...
- (void)setObjectValue:(ActionItem*)item
{
	...
	std::map<bundles::kind_t, NSString*> const map = {
		{ bundles::kItemTypeCommand,     @"命令"      },
		{ bundles::kItemTypeDragCommand, @"拖拽命令" },
		{ bundles::kItemTypeSnippet,     @"片段"      },
		{ bundles::kItemTypeSettings,    @"设置"     },
		{ bundles::kItemTypeGrammar,     @"语法"      },
		{ bundles::kItemTypeProxy,       @"代理"        },
		{ bundles::kItemTypeTheme,       @"主题"        },
		{ bundles::kItemTypeMacro,       @"宏命令"        },
	};
...
- (id)init
{
	if((self = [super init]))
	{
		...
		{
			{ @"标题",          kBundleItemTitleField         },
			{ @"关键等价值", kBundleItemKeyEquivalentField },
			{ @"选项卡触发器",    kBundleItemTabTriggerField    },
			{ @"语义类", kBundleItemSemanticClassField },
			{ @"范围选择器", kBundleItemScopeSelectorField },
		};

		char key = 0;

		[actionMenu addItemWithTitle:@"搜索" action:@selector(nop:) keyEquivalent:@""];
		...
		[actionMenu addItemWithTitle:@"搜索所有范围" action:@selector(toggleSearchAllScopes:) keyEquivalent:key < 9 ? [NSString stringWithFormat:@"%c", '0' + (++key % 10)] : @""];
		...
		self.selectButton             = OakCreateButton(@"选择");
...
- (void)updateItems:(id)sender
{
	...
	self.window.title = [NSString stringWithFormat:@"选择插件项目(%@)", self.itemCountTextField.stringValue];
}
