...
- (id)initWithFrame:(NSRect)aRect
{
	if(self = [super initWithFrame:aRect])
	{
		...
		self.grammarPopUp                 = OakCreateStatusBarPopUpButton(@"", @"语法");
		self.tabSizePopUp                 = OakCreateStatusBarPopUpButton();
		self.tabSizePopUp.pullsDown       = YES;
		self.bundleItemsPopUp             = OakCreateStatusBarPopUpButton(nil, @"插件项");
		self.symbolPopUp                  = OakCreateStatusBarPopUpButton(@"", @"符号");
		self.macroRecordingButton         = OakCreateImageToggleButton(recordMacroImage, @"录制宏命令");
		self.macroRecordingButton.action  = @selector(toggleMacroRecording:);
		self.macroRecordingButton.toolTip = @"点击开始录制宏命令";
		...
		NSTextField* line    = OakCreateTextField(@"行:");
...
- (void)setupTabSizeMenu:(id)sender
{
	MBMenu const items = {
		...
		{ @"其他…",       @selector(showTabSizeSelectorPanel:),  .indent = 1, .target = self.target },
		{ /* -------- */ },
		{ @"缩进按键", @selector(nop:) },
		{ @"Tab键",         @selector(setIndentWithTabs:),         .indent = 1, .target = self.target },
		{ @"Space键",       @selector(setIndentWithSpaces:),       .indent = 1, .target = self.target },
	};
...
- (void)grammarPopUpButtonWillPopUp:(NSNotification*)aNotification
{
	...
	if(grammars.empty())
		[grammarMenu addItemWithTitle:@"没有语法载入" action:@selector(nop:) keyEquivalent:@""];
...
- (void)setSymbolName:(NSString*)newSymbolName
{
	...
	[self.symbolPopUp addItemWithTitle:newSymbolName ?: @"符号"];
}
...
{
	self.tabSizePopUp.title = [NSString stringWithFormat:@"%@:\u2003%lu", _softTabs ? @"软标签" : @"缩进位数", _tabSize];
}
