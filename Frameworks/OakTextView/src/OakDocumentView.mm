...
- (BOOL)validateMenuItem:(NSMenuItem*)aMenuItem
{
	if([aMenuItem action] == @selector(toggleLineNumbers:))
		[aMenuItem setTitle:[gutterView visibilityForColumnWithIdentifier:GVLineNumbersColumnIdentifier] ? @"隐藏行号" : @"显示行号"];
	...
	else if([aMenuItem action] == @selector(showTabSizeSelectorPanel:))
	{
		...
		{
			[aMenuItem setTitle:@"其他…"];
			[aMenuItem setState:NSControlStateValueOff];
		}
		else
		{
			[aMenuItem setDynamicTitle:[NSString stringWithFormat:@"其他 (%zd)…", _textView.tabSize]];
...
- (void)showSymbolSelector:(NSPopUpButton*)symbolPopUp
{
	...
	if(symbolMenu.numberOfItems == 0)
		[symbolMenu addItemWithTitle:@"当前文档无符号显示." action:@selector(nop:) keyEquivalent:@""];
...
- (void)showBundleItemSelector:(NSPopUpButton*)bundleItemsPopUp
{
	...
	if(ordered.empty())
		[bundleItemsMenu addItemWithTitle:@"没有插件载入" action:@selector(nop:) keyEquivalent:@""];
...
- (IBAction)showTabSizeSelectorPanel:(id)sender
{
	if(!tabSizeSelectorPanel)
		[[NSBundle bundleForClass:[self class]] loadNibNamed:@"标签页尺寸设置" owner:self topLevelObjects:NULL];
...
- (NSImage*)imageForLine:(NSUInteger)lineNumber inColumnWithIdentifier:(id)columnIdentifier state:(GutterViewRowState)rowState
{
	if([columnIdentifier isEqualToString:kBookmarksColumnIdentifier])
	{
		...
		[self.document enumerateBookmarksAtLine:lineNumber block:^(text::pos_t const& pos, NSString* type, NSString* payload){
			if(payload.length != 0)
				...
				gutterImageName.emplace(1, rowState != GutterViewRowStateRegular ? @"书签模板" : @"书签模板");
	...
	else if([columnIdentifier isEqualToString:kFoldingsColumnIdentifier])
	{
		...
		{
			case kFoldingTop:       return [self gutterImage:rowState == GutterViewRowStateRegular ? @"顶部折叠模版"       : @"顶部折叠模版"];
			case kFoldingCollapsed: return [self gutterImage:rowState == GutterViewRowStateRegular ? @"折叠模板" : @"折叠模板"];
			case kFoldingBottom:    return [self gutterImage:rowState == GutterViewRowStateRegular ? @"底部折叠模板"    : @"底部折叠模板"];
		}
	}
	return nil;
}
...
- (void)updateBookmarksMenu:(NSMenu*)aMenu
{
	...
	[aMenu addItemWithTitle:@"清除书签" action:hasBookmarks ? @selector(clearAllBookmarks:) : @selector(nop:) keyEquivalent:@""];
}
