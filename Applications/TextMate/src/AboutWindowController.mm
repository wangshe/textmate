...
{
	NSURL* url = [[NSBundle mainBundle] URLForResource:@"更新日志" withExtension:@"html"];
...
- (id)init
{
	...
	{
		_segmentLabels    = @[ @"关于", @"更新日志", @"插件", @"注册", @"法律", @"贡献" ];
		...
		self.toolbar = [[NSToolbar alloc] initWithIdentifier:@"关于文本编辑"];
		...
		[webConfig.userContentController addScriptMessageHandler:self name:@"文本编辑"];
		...
		if(NSURL* url = [NSBundle.mainBundle URLForResource:@"WKWebView" withExtension:@"js"])
		{
			...
			if(NSMutableString* jsBridge = [NSMutableString stringWithContentsOfURL:url encoding:NSUTF8StringEncoding error:&error])
			{
				NSDictionary* variables = @{
					@"版本":   [NSBundle.mainBundle objectForInfoDictionaryKey:@"CFBundleShortVersionString"],
					@"版权": [NSBundle.mainBundle objectForInfoDictionaryKey:@"NSHumanReadableCopyright"],
					@"许可证": LicenseManager.sharedInstance.owner ?: [NSNull null],
				};

				[variables enumerateKeysAndObjectsUsingBlock:^(NSString* key, NSString* value, BOOL* stop){
					[jsBridge appendFormat:@"文本编辑.%@ = %@;\n", key, [self javaScriptEscapedString:[value isEqual:[NSNull null]] ? @"" : value]];
				}];
			...
			{
				os_log_error(OS_LOG_DEFAULT, "WKWebView.js载入失败: %{public}@", error.localizedDescription);
			}
		}
		else
		{
			os_log_error(OS_LOG_DEFAULT, "无法在插件中找到WKWebView.js");
		}
...
{
	self.selectedPage = @"关于";
	[self showWindow:self];
}

- (void)showChangesWindow:(id)sender
{
	self.selectedPage = @"更新日志";
	[self showWindow:self];
...
- (void)setSelectedPage:(NSString*)pageName
{
	...
	NSDictionary* pages = @{
		...
		@"更新日志":       @"About/Changes",
		@"插件":       @"About/Bundles",
		@"注册":  @"About/Registration",
		@"法律":         @"About/Legal",
		@"贡献": @"About/Contributions"
	};
...
{
	if(![_selectedPage isEqualToString:@"插件"])
		return;
