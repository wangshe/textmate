...
- (void)bundlesMenuNeedsUpdate:(NSMenu*)aMenu
{
	...
	if(ordered.empty())
		[aMenu addItemWithTitle:@"没有载入任何插件" action:@selector(nop:) keyEquivalent:@""];
}
...
+ (void)initialize
{
	...
	__weak __block id token = [NSNotificationCenter.defaultCenter addObserverForName:NSApplicationDidFinishLaunchingNotification object:NSApp queue:nil usingBlock:^(NSNotification* notification){
		...
		{
			os_log(OS_LOG_DEFAULT, "从Global.tmProperties中删除旧的主题设置: %{public}@", to_ns(savedThemeUUID));
			...
			if(bundles::item_ptr themeItem = bundles::lookup(savedThemeUUID))
			{
				...
				os_log(OS_LOG_DEFAULT, "将首选外观设置为%{public}@", mode);
...
- (void)themesMenuNeedsUpdate:(NSMenu*)aMenu
{
	...
	if(ordered.empty())
	{
		[aMenu addItemWithTitle:@"没有主题载入" action:@selector(nop:) keyEquivalent:@""];
		return;
	}
	...
	MBMenu const items = {
		{ @"外观",       @selector(nop:),                                                                          },
		{ @"亮色",            @selector(takeThemeAppearanceFrom:), .indent = 1, .target = self, .representedObject = @"light" },
		{ @"暗色",             @selector(takeThemeAppearanceFrom:), .indent = 1, .target = self, .representedObject = @"dark"  },
		{ @"自动",             @selector(takeThemeAppearanceFrom:), .indent = 1, .target = self, .representedObject = nil      },
		{ /* -------- */ },
		{ @"亮色主题", .submenuRef = &lightMenu },
		{ @"暗色主题",  .submenuRef = &darkMenu  },
	};
...
- (void)spellingMenuNeedsUpdate:(NSMenu*)aMenu
{
	...
	NSString* systemSpellingLanguage = [spellChecker automaticallyIdentifiesLanguages] ? @"按语言自动" : NameForLocaleIdentifier([spellChecker language]);
	NSMenuItem* menuItem = [aMenu addItemWithTitle:[NSString stringWithFormat:@"系统(%@)", systemSpellingLanguage] action:@selector(takeSpellingLanguageFrom:) keyEquivalent:@""];
...
- (void)wrapColumnMenuNeedsUpdate:(NSMenu*)aMenu
{
	...
	menuItem = [aMenu addItemWithTitle:@"使用窗口宽度" action:action keyEquivalent:@""];
	...
	[aMenu addItem:[NSMenuItem separatorItem]];
	menuItem = [aMenu addItemWithTitle:@"其他…" action:action keyEquivalent:@""];
